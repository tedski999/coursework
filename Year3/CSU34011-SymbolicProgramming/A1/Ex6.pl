...
% Doesn't work, I've most likely messed up a predicate or two.
% Clearly this entire approach is wrong, so I'm excited to understand
% how this actually should be done.
...
resolve(X-Y,Z) :- resolve(X,A), resolve(Y,B), subtract(A,B,Z).
...
add2_internal(X-Y,Z,W) :- subtract(X,Y,A), add2_internal(A,Z,W).
