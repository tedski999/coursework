...
void listset_copy(struct listset *dst, struct listset *src) {
	...
	// Originally, this function was used to *replace* dst with src, here's the code for if I need it again
	/*
	listset_clear(dst);
	struct listnode *src_node = src->head;
	if (src_node) {
		struct listnode *dst_node = dst->head = malloc(sizeof *dst_node);
		*dst_node = (struct listnode) { src_node->str, NULL };
		while ((src_node = src_node->next)) {
			dst_node->next = malloc(sizeof *dst_node->next);
...
void listset_union(struct listset *dst, struct listset *src1, struct listset *src2) {
	...
	// This function used to *replace* dst with the union of src1 and src2, here's the code for if I need it again
	/*
	struct listset *new_set = listset_new();
	listset_copy(new_set, src1);
	for (struct listnode *cur_node = src2->head; cur_node; cur_node = cur_node->next)
		listset_add(new_set, cur_node->str);
	
	listset_copy(dst, new_set);
	listset_destory(new_set);
	*/
}
...
void listset_intersect(struct listset *dst, struct listset *src1, struct listset *src2) {
	...
	// This function used to *replace* dst with the intersection of src1 and src2, here's the code for if I need it again
	/*
	struct listset *new_set = listset_new();
	for (struct listnode *src1_node = src1->head; src1_node; src1_node = src1_node->next)
		for (struct listnode *src2_node = src2->head; src2_node; src2_node = src2_node->next)
			if (strcmp(src1_node->str, src2_node->str) == 0)
				listset_internal_prepend(new_set, src1_node->str); // no need to check for duplicates, immediately prepend
...
void listset_print(struct listset *set) {
	if (!set)
		return;
