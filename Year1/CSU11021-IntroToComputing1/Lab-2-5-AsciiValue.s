...
; ASCII to value
;
; R0 - Stores input as character
; R1 - Stores the total value of the inputed sequence
; R2 - Stores result of ascii-to-value conversion
; R3 - Stores the significance multiplier (10)
	...
	LDR	R3, =10	; Load significance multiplier

doGet
	BL	get
	BL	put
	
	; Tested with inputs ('1','2','8'), ('0','4','3'), ('9','0','0', '1'), ('L','6','p', '2'),
	; Add next sig. value if input is valid
	CMP	R0, #'0'		;	if (char >= '0'):
	BLO	eIF				;
	CMP	R0, #'9'		;		if (char <= '9'):
	BHI	eIF				;
	SUB	R2, R0, #0x30	;			charValue = char - 0x30
	MUL	R1, R3, R1		;			total = total * 10
	ADD	R1, R1, R2		;			total = total + charValue
eIF						;
